...
const SearchResult = ({ isLoading, state }) => {
    ...
    if (isLoading) {
        searchResultMessage = <p>Loading...</p>;
    } else if (!searchedInstructors) {
        // haven't searched
        searchResultMessage = <p>Search for an instructor</p>;
    } else if (
        Array.isArray(searchedInstructors) &&
        searchedInstructors.length === 0
    ) {
        // no found instructors
        searchResultMessage = <p>No Instructors found</p>;
    } else if (
        Array.isArray(searchedInstructors) &&
        searchedInstructors.length !== 0
    ) {
