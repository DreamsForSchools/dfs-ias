...
const AddClassToProgramModal = ({ handleSubmit, programContext, classData }) => {
    ...
    const [ newClassInput, setClassInput ] = useState(
        {
            ...
            partnerId: classData ? `${classData.partner.partnerId} - ${classData.partner.district ?? ''} ${classData.partner.name}` : '',
    ...
    if (classData) {
        classData.timings.forEach((timing, index) => {
            const startTime = new Date(new Date().toDateString() + ' ' + timing.startTime);
            const endTime = new Date(new Date().toDateString() + ' ' + timing.endTime);
    ...
    useEffect(async () => {
        ...
        try {
            ...
            Object.values(partnerData).forEach((e) => {
                if (!programContext.classes.some(cls => cls.partner.partnerId === e.partnerId) ||
                  (classData && !classData.duplicate && (classData.partner.partnerId === e.partnerId))) {
    ...
    return (
        <>
            <Modal.Header closeButton style={{padding: '2rem 3rem 0 3rem', border: '0'}}>
                <Modal.Title>{ classData ? classData.duplicate ? 'Duplicate the' : 'Edit the' : 'Create a new' } class for {seasonSelected.name}</Modal.Title>
            </Modal.Header>
            <Form noValidate validated={validated} onSubmit={handleSubmitButtonPress}>
                <Modal.Body>
                    <div style={{padding: '2rem 4rem 0 4rem', display: 'flex', flexDirection: 'row'}}>
                        ...
                        <div style={{width: '50%', marginRight: '1.5rem'}} data-testid="classPartners">
                            <Input disabled label={'Program'} state={programContext.name} modal/>
                            <Select required options={partnersList} label={'Partner'} handler={handleFormInput} state={newClassInput.partnerId}
                                    modal/>
                            ...
                            <tr key={idx} style={{ borderRadius: '10px', background: color.neutral.LIGHTGRAY}}>
                                <td>
                                    <input
                                        ...
                                        disabled={classData && classData.duplicate}
                                    /> {e}
                                </td>
                                <td>
                                    {timing[idx].active &&
                                        <div style={{display: 'flex'}}>
                                            ...
                                            <div className={"date-picker-from-control"}>
                                                <label style={{marginLeft: '1rem', marginRight: '0.5rem'}}>To:</label>
                                                <DatePicker
                ...
                <Modal.Footer style={{border: '0', padding: '0 3rem 2rem 3rem'}}>
                    <Button variant="primary" type="submit" data-testid="addClassSubmit">Submit</Button>
                </Modal.Footer>
            </Form>
        </>
    )
